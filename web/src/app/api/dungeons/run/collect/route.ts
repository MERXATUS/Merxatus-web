...
import { tickAndMaybeCollectDungeonRun } from "@/server/dungeonRun";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (dungeon.mode === "PUSH_LUCK") {
      return Response.json({ ok: false, error: "USE_ADVANCE_FOR_PUSH_LUCK" }, { status: 400 });
    }

    const out = await tickAndMaybeCollectDungeonRun({ userId: auth.userId, dungeon, commitLoot: true });
